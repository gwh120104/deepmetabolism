...
# todo: kill this
class ShortAutoEncoder:
    def __init__(self, num_gene, num_pheno, pheno_indices, optimizer=tf.train.AdamOptimizer()):
        # self.pheno_indices = [0, 1, 2]
	self.pheno_indices = pheno_indices
        ...
        self.w1 = tf.Variable(tf.truncated_normal(
            [self.num_gene, self.num_pheno],
            stddev=0.01), name="w1")
        self.b1 = tf.Variable(tf.zeros([self.num_pheno]), name='b1')

        self.w2 = tf.Variable(
            tf.truncated_normal([self.num_pheno, self.num_gene],
                                stddev=0.01), name="w2")
        self.b2 = tf.Variable(tf.zeros(
            [self.num_gene]), name='b2')
        ...
        self.reconstruction_cost = 0.5 * tf.reduce_sum(tf.pow(tf.sub(self.reconstruction, self.gene), 2.0))
        ...
        self.non_zero_actual_pheno = tf.add(self.actual_pheno, 0.00001 * tf.ones(
            tf.shape(self.actual_pheno)))  # Add tiny number to actual phenotype
        # Absolute relative errors
        self.supervised_loss = tf.reduce_sum(
            tf.abs(tf.div(tf.sub(self.non_zero_actual_pheno, self.pred_pheno), self.non_zero_actual_pheno)))
        self.supervised_loss_item = tf.sqrt(
            tf.pow(tf.div(tf.sub(self.actual_pheno, self.pred_pheno), self.actual_pheno), 2.0) / 46.0)
        self.optimizer2 = optimizer.minimize(self.supervised_loss, var_list=[self.b1, self.w1])
    ...
    def get_weight_and_bias(self):
        """ Get the weight matrix from protein to phenotype for growth rate(index 0) """
        return self.sess.run([tf.transpose(self.w1)[0], self.b1[0]], feed_dict={})
    ...
    def supervised_fit(self, gene, pheno):
        cost, opt = self.sess.run((self.supervised_loss, self.optimizer2), feed_dict={
            self.gene: gene,
            self.actual_pheno: pheno})
        return cost

    """ Operators (private) """
    def _half_forward_op(self, X):
        """ Gene->phenotype """
        phenotype = tf.nn.softplus(tf.add(tf.matmul(X, self.w1), self.b1))  # Changed
        return phenotype

    def _forward_op(self, X):
        """ Tensor operator to reconstruct input X """
        phenotype = self._half_forward_op(X)
        reconstructed = tf.matmul(phenotype, self.w2) + self.b2
